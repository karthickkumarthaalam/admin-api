...
const { BlogsCategory, SystemUsers } = db;

exports.createBlogsCategory = async (req, res) => {
  try {
    const { category_name, sub_categories } = req.body;

    const existing = await BlogsCategory.findOne({
      where: { category_name, created_by: req.user.id },
    });
    ...
    const category = await BlogsCategory.create({
      category_name,
      sub_categories,
    ...
    return res.status(500).json({
      status: "error",
      mesage: "Failed to create new category",
      error: error.message,
    });
  }
};
...
exports.getCategoryList = async (req, res) => {
  try {
    ...
    const categories = await BlogsCategory.findAll({
...
exports.getCategories = async (req, res) => {
  try {
    ...
    const result = await pagination(BlogsCategory, {
      page,
      limit,
      where: whereCondition,
      include: [
...
exports.updateCategories = async (req, res) => {
  ...
  try {
    const category = await BlogsCategory.findByPk(id);
