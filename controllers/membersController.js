...
exports.signup = async (req, res) => {
  ...
  try {
    ...
    while (!isUnique) {
      ...
      const existingId = await Members.findOne({
    ...
    await sendNotification(req.app, {
      ...
      message: `${newMember.name} just signed up with email ${newMember.email}`,
      type: "member",
      created_by: newMember.name,
    });
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (!member) {
      return res
        .status(401)
        .json({ status: "error", message: "Invalid mobile/email or password" });
    }
...
exports.getMemberById = async (req, res) => {
  ...
  try {
    if (parseInt(req.user.id) !== parseInt(id)) {
      return res
        .status(403)
        .json({ message: "Unauthorized access to member data" });
    }
...
exports.requestUpdateOtp = async (req, res) => {
  ...
  try {
    ...
    if (!member) {
      return res
        .status(404)
        .json({ status: "error", message: "Member not available" });
    }
...
exports.forgotPassword = async (req, res) => {
  ...
  try {
    if (!email && !phone) {
      return res
        .status(400)
        .json({ status: "error", message: "Please provide email or phone" });
    }
...
exports.resetPassword = async (req, res) => {
  ...
  try {
    if (!newPassword || !confirmPassword) {
      return res
        .status(400)
        .json({ status: "error", message: "Passwords are required" });
    }
