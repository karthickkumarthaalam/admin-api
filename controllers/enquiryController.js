...
exports.createEnquiry = async (req, res) => {
    try {
        ...
        return res.status(201).json({ status: "success", message: "Enquiry created successfully", data: newEnquiry });

    } catch (error) {
        return res.status(500).json({ status: "error", message: "Failed to create enquiry", error: error.message });
    }
};
...
exports.getAllEnquiries = async (req, res) => {
    try {
        ...
        return res.status(200).json({ status: "success", message: "Enquiries fetched successfully", data: result.data, pagination: result.pagination });
    } catch (error) {
        return res.status(500).json({ status: "error", message: "Failed to fetch enquiries", error: error.message });
    }
};
...
exports.updateEnqiryStatus = async (req, res) => {
    try {
        ...
        return res.status(200).json({ status: "success", message: "Enquiry status updated successfully", data: enquiry });
    } catch (error) {
        return res.status(500).json({ status: "error", message: "Failed to update enquiry status", error: error.message });
    }
};
