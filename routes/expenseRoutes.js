...
router.get("/report", expenseController.expensesReport);
...
router.patch(
  "/category/:id/status",
  expenseController.updateExpenseCategoryStatus
);
router.post(
  "/category/:id/add-bill",
  upload.fields([{ name: "bill", maxCount: 1 }]),
  expenseController.updateCategoryBill
);
